...
function CardComp({ item, isVisible, idx }: any) {
    ...
    return (<motion.div
        ...
        viewport={{ once: true }}
        exit={{ opacity: 0 }}
        transition={{ duration: 0.5, delay: .1 * idx }}
    >
                ...
                <div className='p-[.12rem] h-80 flex flex-col items-center justify-center'>
                    {item.poster_path ?
                        <motion.img
                            ...
                            src={fetchImage(item.poster_path)} className="rounded-md object-top h-full object-cover w-full" loading="lazy" alt={item.title.toLowerCase()} />
                        :
            ...
            <AnimatePresence>
                {data[0] && data.slice(0, itemCount).map((item, index) => (
                    <div key={index} className='relative'>
                        ...
                        <CardComp item={item} idx={index} isVisible={cardsLoaded.includes(index)} />
                    </div>
                ))}
            </AnimatePresence>
        </div>
